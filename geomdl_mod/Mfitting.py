...
@export
def approximate_curve(points, degree, kv=None, **kwargs):
    ...
    :param degree: degree of the output parametric curve
    :type degree: int
    :param kv: optional knot vector input
    :type kv: list
...
def approximate_surface(points, size_u, size_v, degree_u, degree_v, **kwargs):
    ...
    # Generate NURBS surface instance
    surf = NURBS.Surface()
    surf.degree_u = degree_u
    surf.degree_v = degree_v
...
def compute_params_curve(points, centripetal=False):
    """ Computes :math:`\\overline{u}_{k}` for curves_v.
